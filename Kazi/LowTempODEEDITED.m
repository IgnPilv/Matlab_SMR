...
global T_0 P_0 r1 k0_1 Ea_1 R rho void_fraction U tube_CSA area_HX delta_H1 delta_H2 Cv_steam m_steam Molar_mass catalyst_diameter flow_area no_tubes V_molar F_H2_in F_H2O_in F_CO_in F_CO2_in F_tot_in
F_H2=z(1);   %mol/s
F_CO=z(2); %mol/s  
F_H2O=z(3);  %mol/s
F_CO2=z(4); %mol/s
...
k1= 1.85.*10^-5.*exp(12.88-1855.5/T);
...
y_CO2 = (F_CO2 / F_tot) ; 
y_CH4 = (F_CH4 / F_tot) ; 
k11= 454.*exp(12.88-1855.5/T)/(0.0283.*3600);
AF= 0.86 + 0.14.*P;
rhobee= 1442;
r1= k11.*AF.*(y_H2O.*y_CO-y_CO2.*y_H2/Kp)/(rhobee.*379);

%% L-H model
p_H2 = (F_H2 / F_tot) .* P; % Pa
p_CO = (F_CO / F_tot) .* P;
p_H2O = (F_H2O / F_tot) .* P;
p_CO2 = (F_CO2 / F_tot) .* P; 
p_CH4 = (F_CH4 / F_tot) .* P;
%k1= 1.188.*exp(-36658/(R.*T_reactor));
%K_CO= 2.283.*10^-24.*exp(-45996/(R.*T_reactor));
%K_H2O= 1.957.*10^-28.*exp(-79963/(R.*T_reactor));
%K_CO2= 5.419.*10^-4.*exp(-16474/(R.*T_reactor));
%K_H2= 2.349.*10^-4.*exp(-13279/(R.*T_reactor));
%k1= 2.215.*10^-4.*exp(-3.783/(R.*T));
K_CO= 1.756.*10^-27.*exp(80.410/R.*(1/T-1/503));
K_H2O= 9.321.*10^-4.*exp(4.109/R.*(1/T-1/503));
K_CO2= 2.280.*10^-3.*exp(9.795/R.*(1/T-1/503));
K_H2= 2.739.*10^-12.*exp(83.608/R.*(1/T-1/503));
y_H2_in = (F_H2_in / F_tot_in) ; % Pa
y_CO_in = (F_CO_in / F_tot_in) ;
y_H2O_in = (F_H2O_in / F_tot_in) ;
y_CO2_in = (F_CO2_in / F_tot_in) ; 
y_CH4 = (F_CH4 / F_tot) ; 
Xeq= 0.9;
%Kp=((y_CO2_in+y_CO_in*Xeq)*(y_H2_in+y_CO_in*Xeq))/((y_CO_in*(1-Xeq)*(y_H2O_in-y_CO_in*Xeq)));
